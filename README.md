...
[![Build calendars](https://github.com/welworx/conferences/actions/workflows/calendars.yml/badge.svg)](https://github.com/welworx/conferences/actions/workflows/calendars.yml)
...
Full list: [https://welworx.github.io/conferences/conferences/all.html#submission-opportunities](https://welworx.github.io/conferences/conferences/all.html#submission-opportunities).

| Deadline | Event | Event Dates | Scope / Co-located | Last Checked |
| --- | --- | --- | --- | --- |
| [Symposium proposals due: 2027-01-31](<https://www.tms.org/TMS2028/TMS2028/Default.aspx>) | [TMS 2028 Annual Meeting & Exhibition (REWAS)](<https://www.tms.org/TMS2028/TMS2028/Default.aspx>) | 2028-03-12 to 2028-03-16 | REWAS | 2026-05-05 |
...
- TMS Annual Meeting & Exhibition:
  `conferences/metallurgy/tms-annual-meeting.yaml`
  - Recorded scoped rows from the pyrometallurgical conferences reference: TMS 2026, 2027, 2028, and 2029.
  - Announced no-date record: TMS 2030, Sulfide Smelting scope, USA.
...
- https://welworx.github.io/conferences/calendars/series/tms-annual-meeting.ics
...
- https://welworx.github.io/conferences/calendars/category/materials.ics
...
- https://welworx.github.io/conferences/calendars/category/pyrometallurgy.ics
...
- https://welworx.github.io/conferences/conferences/all.html

Split conference lists:

- `https://welworx.github.io/conferences/conferences/series/<series-slug>.html`
- `https://welworx.github.io/conferences/conferences/category/<category-slug>.html`
- `https://welworx.github.io/conferences/conferences/domain/<domain-slug>.html`
- `https://welworx.github.io/conferences/conferences/group/<group-slug>.html`

The Markdown conference list has four sections: submission opportunities sorted
by deadline, upcoming events grouped by year, announced conferences whose dates
are still TBD, and past events in reverse chronological order. Events with open
submission deadlines are also repeated in the upcoming section so the future
timeline stays complete. Co-located events are collapsed into the primary event
row with the other tracked conferences linked in parentheses. Location cells
link to Google Maps, using coordinates when available and a location text search
otherwise. The submission status column shows future submission-style deadlines
when known, `Closed` when known submission deadlines have passed, and `TBD` when
no submission deadline is known yet for a future event. The `Last Checked`
column shows the latest explicit source `last_checked` date for the row, or
`TBD` when no checked source date is recorded. Locations are shown as venue and
address lines when those fields are known.

Markdown split lists mirror the generated calendar dimensions for series,
category, domain, and co-located group. Country-specific Markdown lists are not
generated.

RSS event stream:

- https://welworx.github.io/conferences/events.xml

The RSS channel `pubDate` and `lastBuildDate` are generated from the latest Git
commit timestamp. On the published GitHub Pages site, this is effectively the
checked-out `main` commit that produced the site. Individual RSS items do not
set `pubDate` because the source data does not yet track per-event update
timestamps.
...
- `public/conferences/all.html` lists submission opportunities, upcoming events, date-TBD announcements, and past events with links to official pages for browser use.
- `public/conferences/all.md` contains the same content as Markdown.
- `public/conferences/<split>/<slug>.html` and `.md` contain the same structure filtered by series, category, domain, or co-located group.
...
- Use `sources`, event-level `sources`, or deadline-level `sources` lists to
  ...
  descriptions. Add `last_checked: "YYYY-MM-DD"` to a source entry when that
  source was manually checked; the generated Markdown list rolls up the latest
  explicit checked date across the event and its submission-deadline sources.
...
The generator refreshes `public/` Markdown docs and the generated README
submission-opportunities section. The GitHub workflow fails when those generated
docs are stale after a YAML change.
