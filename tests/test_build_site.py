...
class BuildSiteTests(unittest.TestCase):
    ...
    def test_builds_filter_feeds_from_yaml_sources(self) -> None:
        ...
        demo_2027_line = next(line for line in readme_upcoming.splitlines() if "Demo Conference 2027" in line)
        self.assertIn(
            "**10-12**: [Demo Conference 2027](<https://example.org/demo-2027>) "
            "![type: conference](https://img.shields.io/badge/type-conference-blueviolet) "
            "![CFP: due 2026-11-15](https://img.shields.io/badge/CFP-due%202026--11--15-brightgreen)",
            demo_2027_line,
        )
        self.assertNotIn("[![location:", demo_2027_line)
        self.assertNotIn("[![location:", readme_upcoming)
        self.assertNotIn("location-", readme_upcoming)
        self.assertIn(
            "  - **Location:** ![PT flag](https://flagcdn.com/16x12/pt.png) [Lisbon, PT]"
            "(<https://www.google.com/maps/search/?api=1&query=38.7222520,-9.1393370>) | "
            "Venue: Demo Center | Type: conference",
            readme_upcoming,
        )
        self.assertIn("  - **Tags:** `software` `testing` `quality`", readme_upcoming)
        self.assertNotIn("![CFP: TBD]", readme_upcoming)
        demo_2029_detail = next(line for line in readme_upcoming.splitlines() if "**Location:** Location TBD" in line)
        self.assertLess(demo_2029_detail.index("**Location:** Location TBD"), demo_2029_detail.index("Venue: TBD"))
        self.assertLess(demo_2029_detail.index("Venue: TBD"), demo_2029_detail.index("Type: conference"))
    ...
    def test_readme_upcoming_keeps_completed_events_from_current_month(self) -> None:
        items = (
            CalendarItem(
                uid="current-month-2026@industrial-events",
                summary="Current Month Conference 2026",
                start=Date(2026, 6, 2),
                end_exclusive=Date(2026, 6, 4),
                series="Current Month Conference",
                series_slug="current-month",
                domain="software",
                categories=("software",),
                topics=(),
                country="us",
                kind="event",
                status="CONFIRMED",
                url="https://example.org/current-month",
            ),
            CalendarItem(
                uid="previous-month-2026@industrial-events",
                summary="Previous Month Conference 2026",
                start=Date(2026, 5, 28),
                end_exclusive=Date(2026, 5, 30),
                series="Previous Month Conference",
                series_slug="previous-month",
                domain="software",
                categories=("software",),
                topics=(),
                country="us",
                kind="event",
                status="CONFIRMED",
                url="https://example.org/previous-month",
            ),
        )
        ...
        self.assertIn("Current Month Conference 2026", readme_upcoming)
        self.assertNotIn("Previous Month Conference 2026", readme_upcoming)
